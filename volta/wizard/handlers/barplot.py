...
class BarplotBuilder(tornado.web.RequestHandler):
    def get(self):
        ...
        items = ['./logs/{filename}'.format(filename=filename) for filename in dir_files if filename.startswith('current_')]
        self.render(
...
def input_files_to_df(files, delimiter):
    ...
    for work_file in files:
        ...
        if work_df is None:
            work_df = pd.read_csv(work_file, delimiter=delimiter, names="date test_id ts curr".split())
            work_df['label'] = '{name}'.format(name=filename)
        else:
            df = pd.read_csv(work_file, delimiter=delimiter, names="date test_id ts curr".split())
            df['label'] = '{name}'.format(name=filename)
            work_df = work_df.append(df)
    # convert unixtimestamp to datetime/s
    work_df['ts'] = work_df.ts.astype(int)//10000
    work_df['ts'] = pd.to_datetime(work_df['ts'] ,unit='s')
    work_df['curr'] = pd.to_numeric(work_df['curr'], errors='coerce')    
    return work_df
